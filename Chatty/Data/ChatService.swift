...
class ChatService {
    ...
    func sendMessage(_ messageText: String, isImage: Bool?, isVideo: Bool?, isAudio: Bool?) {
        ...
        let latestCurrentUserRef = FirestoreConstants.messageCollection.document(currentUid).collection(CollectionPath.latestMessages).document(chatPartnerId)
        let latestPartnerRef = FirestoreConstants.messageCollection.document(chatPartnerId).collection(CollectionPath.latestMessages).document(currentUid)
